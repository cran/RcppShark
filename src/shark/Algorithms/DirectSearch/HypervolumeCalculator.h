...
namespace shark {
        ...
        */
        ...
        double HypervolumeCalculator::getMeasure( const VectorType & regionLow, const VectorType & regionUp ) {
                ...
                // Rcpp::Rcout << "Get Measure: " << volume << std::endl;
        ...
        double HypervolumeCalculator::stream( const VectorType & regionLow,
                ...
                double cover ) {
                        ...
                        //  Rcpp::Rcout << "(II) No points: " << points.size() << std::endl;

                        for (c = coverIndex; c > 0; c--) {
                                // if (points[(c - 1) * NO_OBJECTIVES + NO_OBJECTIVES - 1] == cover) {
                                /*Rcpp::Rcout << "\t " << extractor( points[c-1] )[m_noObjectives-1] << " vs. " << cover << std::endl;
                                if( c == 1 )
                                Rcpp::Rcout << "\t\t " << extractor( points[c-1] )[m_noObjectives-1] << " vs. " << cover << std::endl;*/
                        ...
                        // Rcpp::Rcout << "Cover index: " << coverIndex << ", split: " << split << std::endl;
                        ...
                        if( allPiles ) {                
                                ...
                                do {
                                        ...
                                        if (boundIdx > 0) {
                                                ...
                                                // Rcpp::Rcout << "Median: " << bound << std::endl;
